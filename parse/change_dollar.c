...
/*   Updated: 2023/02/18 17:43:29 by tyenokya         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*change_doll(char *str, t_env **en)
{
	...
	while (str && str[i])
	{
		print_doll(str, &i);
		if (str[i] == '$' && str[i + 1] &&
			(str[i + 1] == '_' || ft_isalpha(str[i + 1]) || str[i + 1] == '?'))// ?-i depqy woncor grac chi
		{
			start = i;
			if (str[i] && !print_doll(str, &i))
			{
				++i;
				while (str[i] && (str[i] != '$' || !is_meta(str[i]) || str[i] == '\"')) //metacharacterov petqa woncor stugel
					++i;
				end = i;
